...
public:
  ...
  void deepCopy(const resourceInfo_t *resourceInfo, int idx) {
    ...
    cppDeliteArrayint32_t * *old = data;

    // Stop using temporary heap
    StopTempHeap(resourceInfo->threadId);
    // Allocate a new chunk of memory in the permanent heap for the data
    data = (cppDeliteArrayint32_t * *)DeliteHeapAlloc(length * sizeof(data[0]), resourceInfo->threadId, true);   
    // Continue the use of the temporary heap
    StartTempHeap(resourceInfo->threadId);

    // Copy data into the newly allocated memory
    memcpy(data, old, length * sizeof(old[0]));

    // Move each element of the array to the permanent heap as it is called
    data[idx] = (cppDeliteArrayint32_t *)old[idx]->deepCopy(resourceInfo, idx);
  }
#endif
