...
class SendingConnection(val address: InetSocketAddress, selector_ : Selector,
      ...
      messages.synchronized {
        while (!messages.isEmpty) {
          ...
          if (chunk.isDefined) {
            ...
            if (!message.started) {
              log("Starting to send [" + message + "] to [" + remoteConnectionManagerId + "]")
              message.started = true
              message.startTime = System.currentTimeMillis
            }
  ...
  def connect() {
    try{
      ...
      log("Initiating connection to [" + address + "]")
    } catch {
      case e: Exception => {
        log("Error connecting to " + address, e)
  ...
  def finishConnect() {
    try {
      ...
      changeConnectionKeyInterest(SelectionKey.OP_WRITE)
      log("Connected to [" + address + "], " + outbox.messages.size + " messages pending")
    } catch {
      case e: Exception => {
        log("Error finishing connection to " + address, e)
        callOnExceptionCallback(e)
      }
    }
  }
