...
trait ScheduleOptimizer {
  ...
  // Schedules C++ kernels on FPGA host, removing Scala kernel dependency from FPGA execution
  // TODO: Needs to consider executions with multiple CUDA or C++ schedules
  def scheduleNativeFPGA(graph: DeliteTaskGraph) {
    ...
    for (opList <- graph.schedule if OpHelper.scheduledTarget(opList.resourceID) == Targets.Scala) {
      ...
      if (opList.size > 0)
        println("WARNING (scheduleNativeFPGA): Configured to run FPGA natively, but still " + opList.size + " scala kernels exist.")
    }
    ...
    sortedSchedule.foreach { _ match {
      ...
      case nested: OP_Nested =>
        // recursively schedule nested graphs
        nested.nestedGraphs.foreach(scheduleNativeFPGA)
