...
object MultiLoop_GPU_Array_Generator extends JNIFuncs {
  ...
  private def hashReductionList(op: OP_MultiLoop): List[(OPData,String)] = {
    op.getGPUMetadata(Targets.Cuda).outputs.filter(o => o._1.loopType=="HASH_REDUCE" || o._1.loopType=="HASH_REDUCE_SPEC")
  }
  ...
  private def makeKernels(out: StringBuilder, op: OP_MultiLoop) {
    ...
    if(hashReductionList(op).filter(o => o._1.loopType=="HASH_REDUCE").nonEmpty)
      writeHashReduceKernel(out,op,false)
    if(hashReductionList(op).filter(o => o._1.loopType=="HASH_REDUCE_SPEC").nonEmpty)
  ...
  private def writeKernelCall(out: StringBuilder, op: OP_MultiLoop, id: String) {
    ...
    id match {
      ...
      case "HashReduce" | "HashReduceSpec" =>
        if(id == "HashReduce") {
          val osym = hashReductionList(op).filter(_._1.loopType == "HASH_REDUCE")(0)._2
          out.append(cudaLaunch(dimSize("host_offset_"+osym+"[0],1,1")))
        }
        else {
          val osym = hashReductionList(op).filter(_._1.loopType == "HASH_REDUCE_SPEC")(0)._2
  ...
  private def writeKernelHeader(out: StringBuilder, op: OP_MultiLoop, id: String) {
    ...
    val params = op.getGPUMetadata(Targets.Cuda).outputs.filter(o => !isPrimitiveType(op.outputType(o._2))).map(o => op.outputType(Targets.Cuda, o._2) + " " + o._2) ++ conditionList(op).map(o => "unsigned int * bitmap_" + o._2 + ", unsigned int * scanmap_" + o._2) ++ reductionList(op).map(o => o._1.loopFuncOutputType + " *temp_" + o._2 + "," + o._1.loopFuncOutputType + " *temp_" + o._2 + "_2") ++ (reductionSpecList(op).map(o => o._1.loopFuncOutputType_2 + " *temp_" + o._2 + "," + o._1.loopFuncOutputType_2 + " *temp_" + o._2 + "_2," + o._1.loopFuncOutputType + " *tempIn_" + o._2)) ++ reductionTupleList(op).map(o => o._1.loopFuncOutputType + " *temp1_" + o._2 + "," + o._1.loopFuncOutputType + " *temp1_" + o._2 + "_2," + o._1.loopFuncOutputType_2 + " *temp2_" + o._2 + "," + o._1.loopFuncOutputType_2 + " *temp2_" + o._2 + "_2") ++ hashReductionList(op).map(o => keyType + " *key_" + o._2 + "," + o._1.loopFuncOutputType + " *val_" + o._2 + ", int *offset_" + o._2 + ", int *idx_" + o._2)
  ...
  private def writeReduceSpecKernel(out: StringBuilder, op: OP_MultiLoop) {
    writeKernelHeader(out, op, "ReduceSpec")  
      ...
      out.append("while( idxX < " + opSize + ") {\n")
      for((odata,osym) <- reductionSpecList(op)) {
        out.append("localSum_" + osym + " = dev_combine_" + funcNameSuffix(op,osym) + (odata.loopReduceInputs++List("localSum_"+osym,"tempIn_"+osym+"[idxX].dc_apply(blockIdx.x)")++lastInputArgs(op)).mkString("(",",",");\n"))
      }
      out.append("idxX += blockSize;\n")
      out.append("}\n")
      for((odata,osym) <- reductionSpecList(op)) {
  ...
  */
  ...
  private def writeHashReduceKernel(out: StringBuilder, op: OP_MultiLoop, unrolled: Boolean) {
    val symList = if(unrolled) hashReductionList(op).filter(_._1.loopType=="HASH_REDUCE_SPEC")
                  else hashReductionList(op).filter(_._1.loopType=="HASH_REDUCE")
    if(unrolled) 
    ...
    out.append("idxX = threadIdx.x + start;\n")
      ...
      for((odata,osym) <- symList) {
        if(unrolled)
          out.append("localSum_" + osym + " = dev_combine_" + funcNameSuffix(op,osym) + (odata.loopReduceInputs++List("localSum_"+osym,"val_"+osym+"[idx_" + osym + "[idxX]].dc_apply(blockIdx.y)")++lastInputArgs(op)).mkString("(",",",");\n"))
        else
  ...
  private def writeHashReducePreKernelCall(out: StringBuilder, op: OP_MultiLoop) {   
    for ((odata,osym) <- hashReductionList(op)) {
      ...
      if(odata.loopType=="HASH_REDUCE_SPEC") {
        ...
        out.append("DeliteCudaMemcpyDtoHAsync((void*)&res_" + osym + ", (void*)val_"+osym+", sizeof(" + odata.loopFuncOutputType + "));\n")
        out.append(odata.loopFuncOutputType + " *x = res_" + osym + ".dc_alloc();\n")
        out.append("DeliteCudaMemcpyHtoDAsync((void*)(((*" + osym + ")->data)+i), (void*)x, sizeof(" + odata.loopFuncOutputType + "));\n")
        out.append("}\n")
      }
  ...
  private def allocateTemps(out: StringBuilder, op: OP_MultiLoop) {
    ...
    for ((odata,osym) <- hashReductionList(op)) {
      out.append(keyType + " *key_" + osym + ";\n")
      out.append(odata.loopFuncOutputType + " *val_" + osym + ";\n")
      out.append("int *offset_" + osym + ";\n")
      out.append("int *idx_" + osym + ";\n")
      out.append("DeliteCudaMallocTemp((void**)&key_" + osym + ", " + opSize + "*sizeof(" + keyType + "));\n")
      out.append("DeliteCudaMallocTemp((void**)&val_" + osym + ", " + opSize + "*sizeof(" + odata.loopFuncOutputType + "));\n")
