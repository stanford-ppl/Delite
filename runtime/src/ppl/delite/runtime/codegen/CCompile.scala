...
trait CCompile extends CodeCache {
  ...
  def makefileString(destination: String, sources: Array[String], includes: Array[String], libs: Array[String], features:Array[String]) = """
...
%%.o: %%.%9$s
	$(CC) -c -DDELITE_CPP=%11$s -DMEMMGR_%12$s %13$s $(INCLUDES) $(CFLAGS) $< -o $@
...
""".format(config.compiler,Config.deliteHome,sourceCacheHome,binCacheHome,includes.mkString(" "),
           ...
           Config.numCpp,Config.cppMemMgr.toUpperCase, features.map("-D"+_).mkString(" "))
}
