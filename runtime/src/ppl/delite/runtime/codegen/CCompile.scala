...
trait CCompile extends CodeCache {
  ...
  def compile() {
    ...
    compile(dest, sourceDirs ++ shared.sourceDirs)
  ...
  def compile(destination: String, sources: Array[String]) {
    ...
    if(!Config.noRegenerate) {
      ...
      writer.write(makefileString(destination, srcFiles, srcDirs, optionalFeatures))
      writer.close()
    }
  ...
  def makefileString(destination: String, srcFiles: Array[String], srcDirs: Array[String], features:Array[String]) = s"""
