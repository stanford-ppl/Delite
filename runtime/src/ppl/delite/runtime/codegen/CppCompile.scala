...
object CppCompile extends CCompile {
  ...
  override def compile(destination: String, sources: Array[String], paths: Array[String]) {
    ...
    if(!Config.noRegenerate) {	  
      ...
      writer.write(makefileString(paths))
      writer.close()
    }
    ...
    val args = Array(config.make, "-j", Runtime.getRuntime.availableProcessors.toString, "-f", makefile, "clean", "all")
  ...
  def makefileString(paths: Array[String]) = """
## Makefile: Generated by Delite Runtime ##
CC = %1$s
SOURCECACHE_HOME = %2$s
BINCACHE_HOME = %3$s
INCLUDES = %4$s

CFLAGS = -O3 -fPIC -w
LDFLAGS = -shared -fPIC
SOURCES = $(wildcard $(SOURCECACHE_HOME)/kernels/*.cpp) $(wildcard $(SOURCECACHE_HOME)/runtime/*.cpp)
OBJECTS = $(SOURCES:.cpp=.o)
OUTPUT = $(BINCACHE_HOME)/cppHost.so
...
.PHONY: all clean
""".format(config.compiler,sourceCacheHome,binCacheHome,paths.mkString(" "))
