...
class DNodeModel(val id: Int, val parentName: Types.DNodeName,
                 val targetsSupported: Int, val level: Int,
                 val _type: DNodeType.Value,
                 val inputNodes: ArrayBuffer[String],
                 val outputNodes: ArrayBuffer[String],
                 val childNodes: ArrayBuffer[String])
{ }

class ProfilerSuite extends Suite {

    val sep = "/" 
    val inputsDirectory = "/home/jithinpt/cache_instrumentation/hyperdsl/delite/runtime/tests/ppl/tests/scalatest/inputs/ProfilerSuite/input_1"
    ...
    def checkDNode(depGraph: DependencyGraph, name: String,
                   ...
                   childNodes: ArrayBuffer[String])
   	{
   		...
   		val model = new DNodeModel(id, parentName, targetsSupported, level, _type, inputNodes, outputNodes, childNodes)
	...
	def checkExecutionSummary(executionProfile: ExecutionProfile, name: Types.TNodeName,
    	...
    	assertResult(true, " [No summary found for " + name + "]")(false)
    	return
    }
