...
trait StencilExp extends LoopsExp {
  ...
  object unknown extends AccessPattern { override def toString = "unknown" }
    ...
    def processArrayAccess(a: Exp[DeliteArray[Any]], i: Exp[Int]) {
      ...
      if (v == i) {
        ...
        i match {
          ...
          case s:Sym[_] if findDefinition(s).isEmpty =>
            // We are using the lack of a definition here as evidence that the index is some bound symbol, which may not be precise.
            // To use boundSyms(..), we need the enclosing definition that constructed the symbol.
            // TODO: we should extend this to expression trees that contain some other bound sym s where s != v
            log("    determining that array " + a.toString + " accesses elements at some non-interval bound index " + s.toString + ". assuming all elements are accessed.")
            stencil += a -> all
          case _ =>
            log("    could not determine how array " + a.toString + " is accessed as a function of loop index " + v.toString + ".")
            stencil += a -> unknown
        }
      }
    }
