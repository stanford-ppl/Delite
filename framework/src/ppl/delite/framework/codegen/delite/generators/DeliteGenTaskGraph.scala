...
trait DeliteGenTaskGraph extends DeliteCodegen {
  ...
  override def emitNode(sym: Sym[_], rhs: Def[_])(implicit stream: PrintWriter) : Unit = {
    ...
    rhs match {
      ...
      //case IfThenElse(c,t,e) => emitBlock(t); emitBlock(e); skipEmission = true
    ...
    if (supportedTargets.isEmpty) system.error("Node " + quote(sym) + "[" + rhs + "] could not be generated by any code generator")
    ...
    */
    ...
    rhs match {
      //case IfThenElse(c,t,e) => emitIfThenElse(c, t, e, symt, inputs, inControlDeps, antiDeps)
