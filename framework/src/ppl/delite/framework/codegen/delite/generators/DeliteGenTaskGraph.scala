    ...
    */
    ...
    rhs match {
      case op: AbstractFatLoop =>
        ...
        emitMultiLoop(kernelName, outputs, inputs, inMutating, inControlDeps, antiDeps, op.size, op.body.exists (loopBodyNeedsCombine _), op.body.exists (loopBodyNeedsPostProcess _), optContext, opStencil)
      ...
      case z =>
        z match {
          case op:AbstractLoop[_] => 
            ...
            // Predef.println(inputs)          
            // Predef.println("stencil is: " + opStencil)
            emitMultiLoop(kernelName, outputs, inputs, inMutating, inControlDeps, antiDeps, op.size, loopBodyNeedsCombine(op.body), loopBodyNeedsPostProcess(op.body), optContext, opStencil)
