...
//      }
  ...
  override def mirror[A:Manifest](e: Def[A], f: Transformer): Exp[A] = (e match {
    case DeliteIfThenElse(c,a,b) => reflectPure(DeliteIfThenElse(f(c),f(a),f(b)))(mtype(manifest[A]))
    case Reflect(DeliteIfThenElse(c,a,b), u, es) => reflectMirrored(Reflect(DeliteIfThenElse(f(c),f(a),f(b)), mapOver(f,u), f(es)))(mtype(manifest[A]))
