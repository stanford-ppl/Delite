...
trait DeliteCGenVariables extends CGenEffect with DeliteCLikeGenVariables {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any])(implicit stream: PrintWriter) = {
    ...
    if (symIsResult) {
      rhs match {
        case NewVar(init) => emitValDef(sym, quote(getBlockResult(init))); gen = true
        case _ => // pass
      }
    }
    if (!(deliteInputs intersect syms(rhs)).isEmpty) {
      rhs match {
        ...
        case Assign(Variable(a), b) => stream.println(quote(a) + " = " + quote(getBlockResult(b)) + ";"); gen = true
        case VarPlusEquals(Variable(a), b) => stream.println(quote(a) + " += " + quote(getBlockResult(b)) + ";"); gen = true
        case VarMinusEquals(Variable(a), b) => stream.println(quote(a) + " -= " + quote(getBlockResult(b)) + ";"); gen = true
        case _ => // pass
      }
    }
