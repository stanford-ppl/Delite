...
trait Emitter {
  ...
  def emitBlock(start: Exp[_], target: Target{val intermediate: Emitter.this.intermediate.type})(implicit stream: PrintWriter): Unit = {
  ...
  def findSyms(e: Any, target: Target{val intermediate: Emitter.this.intermediate.type}, shallow: Boolean): List[Sym[Any]] = {
  ...
  def dep2(e: Exp[Any], target: Target{val intermediate: Emitter.this.intermediate.type}, shallow: Boolean): List[Sym[Any]] = e match {
  ...
  // from scheduling
  def buildScheduleForResult(start: Exp[_], target: Target{val intermediate: Emitter.this.intermediate.type}, shallow: Boolean): List[TP[_]] = {
