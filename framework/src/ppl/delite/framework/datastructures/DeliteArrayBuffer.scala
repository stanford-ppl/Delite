...
trait DeliteArrayBufferOpsExp extends DeliteArrayBufferOps with DeliteCollectionOpsExp with DeliteStructsExp {
  ...
  // (Possibly) mutable buffer with preallocation -- internal use only
  ...
  def darray_buffer_insertspace[A:Manifest](d: Exp[DeliteArrayBuffer[A]], pos: Exp[Int], len: Exp[Int])(implicit ctx: SourceContext): Exp[Unit] = {
