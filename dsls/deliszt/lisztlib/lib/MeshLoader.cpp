...
MeshLoader::MeshLoader(JNIEnv* env) : env(env), cache(env) {
...
jobject MeshLoader::callObjectMethod(jobject obj, string clsStr, string method, string sig, ...) {
  va_list args;
  va_start(args, type);
...
jobject MeshLoader::callObjectMethodV(jobject obj, string clsStr, string method, string sig, va_list args) {
  jclass cls;
  if(!(cls = cache.getClass(clsStr))) {
    throw new MeshLoadException("Failed to find class " + clsStr); /* exception thrown */
  }
  ...
  if (cid == NULL) {
    throw new MeshLoadException("Failed to find method " + method + " with sig " + sig); /* exception thrown */
  }
  
  return env->CallObjMethodV(jobj, cid, args);
}

jobject MeshLoader::callVoidMethod(jobject obj, string clsStr, string method, string sig, ...) {
  va_list args;
  va_start(args, type);
...
jobject MeshLoader::callVoidMethodV(jobject obj, string clsStr, string method, va_list args) {
  jclass cls;
  if(!(cls = cache.getClass(clsStr))) {
    throw new MeshLoadException("Failed to find class " + clsStr); /* exception thrown */
  }
...
jobject MeshLoader::getScalaObjField(string clsStr, jobject jobj, string field, string type) {
...
jobject MeshLoader::getScalaObjField(jclass cls, jobject jobj, string field, string type) {
  jmethodID cid;
  string getter = field;
  string sig = "()" + type;
  ...
  return env->CallObjMethod(jobj, cid);
}

void MeshLoader::setScalaField(jclass cls, jobject jobj, string field, string type, ...) {
  va_list args;
  va_start(args, type);
  jmethodID cid;
...
void MeshLoader::setCRSField(jobject jmesh, string field, CRSMeshPrivate::CRS& crs, size_t from) {   
  jintArray row_idx = copyIdxArray(crs.row_idx, from);
  jintArray values = copyIdArray(crs.values, crs.row_idx[from]);

  jobject jcrs = createObject("ppl/dsl/deliszt/datastruct/scala/CRS", "[I[I", row_idx, values);
  
  setScalaField(meshClass, jmesh, field, "ppl/dsl/deliszt/datastruct/scala/CRS", jcrs);
}

void MeshLoader::setCRSConstField(jobject jmesh, string field, CRSMeshPrivate::CRSConst& crs, size_t from, size_t mult) {   
  jintArray values = copyIdPairArray(crs.values, from * mult);

  jobject jcrs = createObject("ppl/dsl/deliszt/datastruct/scala/CRSConst", "[II", values, mult);
  
  setScalaField(meshClass, jmesh, field, "ppl/dsl/deliszt/datastruct/scala/CRSConst", jcrs);
}

jintArray MeshLoader::copyIdxArray(CRSMeshPrivate::idx_type* array, size_t len) {
...
jintArray MeshLoader::copyIdPairArray(CRSMeshPrivate::IDPair* array, size_t len) {
...
int MeshLoader::loadPositions(jobject& jmesh, CRSMesh::Mesh& mesh, MeshIO::LisztFileReader& reader) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    jdoubleArray farr = env->NewDoubleArray(size);
    if (darr == NULL) {
    ...
    env->SetIntArrayRegion(farr, 0, 3, tmp);
    env->SetObjectArrayElement(positions, i, farr);
    env->DeleteLocalRef(farr);
  }

	reader.free(table);
  
  jobject vertexLabels = getScalaObjField(meshClass, jmesh, "vertexData", "Lppl/dsl/deliszt/datastruct/scala/LabelData");
  jclass labelClass = cache.getClass("ppl/dsl/deliszt/datastruct/scala/LabelData");
  jobject vertexData = getScalaObjField(labelClass, vertexLabels, "Lscala/collection/mutable/Map");
  
  callObjectMethod(vertexLabels, "scala/collection/mutable/Map", "put", "(Ljava/lang/Object,Ljava/lang/Object)Lscala/Option", env->NewStringUTF("position"), positions);
}

void MeshLoader::loadBoundaries(jobject& jmesh, CRSMesh::Mesh& mesh, MeshIO::LisztFileReader& reader) {
  ...
  for(size_t i = 0; i < h.nBoundaries; i++) {
    ...
    if((bounds[i].name.size() > 0) {
      MeshIO::IOElemType typ = (MeshIO::IOElemType)(bounds[i].type & ~MeshIO::AGG_FLAG);

      switch(typ) {
        case LisztPrivate::ElemTypes::CELL:
          loadBoundarySet<Cell, cell_set>(jmesh, mesh, bounds[i].name.c_str(), "cellBounds");
          break;
        case LisztPrivate::ElemTypes::EDGE:
          loadBoundarySet<Edge, edge_set>(jmesh, mesh, bounds[i].name.c_str(), "edgeBounds");
          break;
        case LisztPrivate::ElemTypes::FACE:
          loadBoundarySet<Face, face_set>(jmesh, mesh, bounds[i].name.c_str(), "faceBounds");
          break;
        case LisztPrivate::ElemTypes::VERTEX:
          loadBoundarySet<Vertex, vertex_set>(jmesh, mesh, bounds[i].name.c_str(), "vertexBounds");
          break;
        default:
          break;
      }
