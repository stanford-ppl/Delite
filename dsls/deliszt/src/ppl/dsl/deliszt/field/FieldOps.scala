...
trait FieldOpsExpOpt extends FieldOpsExp {
  ...
  override def field_mo_update[MO<:MeshObj:Manifest,T:Manifest](x: Exp[Field[MO,T]], mo: Rep[MO], v: Exp[T]) = v match {
    case Def(ArithPlus(a, b)) => (a, b) match {
        case (a, Def(FieldApply(`x`, `mo`))) => FieldPlusUpdate(x, mo.asInstanceOf[Exp[Int]], a)
        case (Def(FieldApply(`x`, `mo`)), b) => FieldPlusUpdate(x, mo.asInstanceOf[Exp[Int]], b)
        case _ => super.field_mo_update(x, mo, v)
    }
    case Def(ArithTimes(a, b)) => (a, b) match {
        case (a, Def(FieldApply(`x`, `mo`))) => FieldTimesUpdate(x, mo.asInstanceOf[Exp[Int]], a)
        case (Def(FieldApply(`x`, `mo`)), b) => FieldTimesUpdate(x, mo.asInstanceOf[Exp[Int]], b)
        case _ => super.field_mo_update(x, mo, v)
    }
    case Def(ArithMinus(Def(FieldApply(`x`, `mo`)), b)) => FieldMinusUpdate(x, mo.asInstanceOf[Exp[Int]], b)
    case Def(ArithFractionalDivide(Def(FieldApply(`x`, `mo`)), b)) => FieldDivideUpdate(x, mo.asInstanceOf[Exp[Int]], b)
