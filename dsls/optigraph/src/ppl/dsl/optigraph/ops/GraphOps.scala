...
trait GraphOpsExp extends GraphOps with EffectExp with NodeOps {
  ...
  override def mirror[A:Manifest](e: Def[A], f: Transformer)(implicit ctx: SourceContext): Exp[A] = (e match {
    ...
    case Reflect(e@GraphNodeOutNeighbors(g,n), u, es) => reflectMirrored(Reflect(GraphNodeOutNeighbors(f(g),f(n)), mapOver(f,u), f(es)))(mtype(manifest[A])) //reflectMirrored(Reflect(new { override val original = Some(f,e) } with GraphNodeOutNeighbors(f(g),f(n)), mapOver(f,u), f(es)))(mtype(manifest[A]))
    ...
    case Reflect(e@GraphNodes(g), u, es) => reflectMirrored(Reflect(new { override val original = Some(f,e) } with GraphNodes(f(g)), mapOver(f,u), f(es)))(mtype(manifest[A]))
