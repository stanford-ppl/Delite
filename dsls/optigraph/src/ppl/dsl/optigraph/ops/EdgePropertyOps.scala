...
trait EdgePropertyOpsExp extends EdgePropertyOps with VariablesExp with BaseFatExp {
  ...
  override def mirror[A:Manifest](e: Def[A], f: Transformer)(implicit ctx: SourceContext): Exp[A] = (e match {
    case EdgePropGetDef(ep,i) => edgeprop_get_def(f(ep),f(i))
    ...
    case Reflect(e@EdgePropGetDef(ep,i), u, es) => reflectMirrored(Reflect(EdgePropGetDef(f(ep),f(i)), mapOver(f,u), f(es)))(mtype(manifest[A]))
