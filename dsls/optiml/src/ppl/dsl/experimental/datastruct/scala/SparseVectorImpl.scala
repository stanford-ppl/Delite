 ...
 */
//class SparseVectorImpl[@specialized T: ClassManifest](__length: Int, __isRow: Boolean) extends SparseVector[T] {
class SparseVector[@specialized T: ClassManifest](__length: Int, __isRow: Boolean) {
  ...
  def cloneL = { val v = new SparseVector[T](0, isRow); v.insertAll(0, this); v }
  ...
  def insertAll(pos: Int, xs: SparseVector[T]) {
    insertSpace(pos, xs.length)
    copyFrom(pos, xs)
  }

  def copyFrom(pos: Int, xs: SparseVector[T]) {
