...
trait MLOutputWriterOps extends Base {
  object MLOutputWriter {
    def writeImgPgm(img: Rep[Matrix[Double]], filename: Rep[String])(implicit ctx: SourceContext) = obj_mloutput_write_img_pgm(img,filename)
  }

  def obj_mloutput_write_img_pgm(img: Rep[Matrix[Double]], filename: Rep[String])(implicit ctx: SourceContext): Rep[Unit]
}
...
trait MLOutputWriterOpsExp extends MLOutputWriterOps { this: MLOutputWriterImplOps with DeliteOpsExp =>
  ...
  def obj_mloutput_write_img_pgm(img: Exp[Matrix[Double]], filename: Exp[String])(implicit ctx: SourceContext) = reflectEffect(MLOutputWriteImgPgm(img,filename))
}
