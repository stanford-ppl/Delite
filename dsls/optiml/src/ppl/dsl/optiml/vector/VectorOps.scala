...
trait VectorOpsExpOpt extends VectorOpsExp with DeliteCollectionOpsExp {
  ...
  override def vector_mutable_clone[A:Manifest](x: Exp[Vector[A]]) = x match {
    ...
    //case Def(d@VectorObjectZeros(len)) => reflectMutable(d.asInstanceOf[Def[Vector[A]]]) <--- problem with RBM: reflectMutable will not duplicate the alloc that's inside the single task
