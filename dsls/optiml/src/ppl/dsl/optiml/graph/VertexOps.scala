...
trait VertexOps extends Variables {
  ...
  class VertexOpsCls[VD:Manifest,ED:Manifest](v: Rep[Vertex[VD,ED]]) {
    ...
    def setData(x: Rep[VD]) = vertex_set_data(v,x)
  ...
  def vertex_set_data[VD:Manifest,ED:Manifest](v: Rep[Vertex[VD,ED]], data: Rep[VD]): Rep[Unit]
...
trait VertexOpsExp extends VertexOps with EffectExp {
  ...
  case class VertexSetData[VD:Manifest,ED:Manifest](v: Exp[Vertex[VD,ED]], data: Exp[VD]) extends Def[Unit]  
  ...
  // the graph needs to store mutable instances of vertices and edges...is this technically allowed? (immutable collections of mutable instances)
  // currently, we are working around this by being extremely coarse-grained: any read or write to any vertex must serialize with all others.
  
  def vertex_new[VD:Manifest,ED:Manifest](g: Exp[Graph[VD,ED]], d: Exp[VD]) = reflectMutable(VertexNew(g.unsafeImmutable,d))
  ...
  def vertex_tasks[VD:Manifest,ED:Manifest](v: Exp[Vertex[VD,ED]]) = reflectEffect(VertexTasks(v), Global()) //reflectPure(VertexTasks(v))
  def vertex_clear_tasks[VD:Manifest,ED:Manifest](v: Exp[Vertex[VD,ED]]) = reflectEffect(VertexClearTasks(v), Global()) //reflectWrite(v)(VertexClearTasks(v))
  def vertex_add_task[VD:Manifest,ED:Manifest](v: Exp[Vertex[VD,ED]], t: Exp[Vertex[VD,ED]]) = reflectEffect(VertexAddTask(v, t), Global()) //reflectWrite(v)(VertexAddTask(v, t))  
  ...
  def vertex_get_data[VD:Manifest,ED:Manifest](v: Exp[Vertex[VD,ED]]) = reflectEffect(VertexGetData(v), Global()) //reflectPure(VertexGetData(v))
  def vertex_get_tasks[VD:Manifest,ED:Manifest](v: Exp[Vertex[VD,ED]]) = reflectEffect(VertexGetTasks(v), Global()) //reflectPure(VertexGetTasks(v))
  
  def vertex_set_data[VD:Manifest,ED:Manifest](v: Exp[Vertex[VD,ED]], data: Exp[VD]) = reflectEffect(VertexSetData(v,data), Global()) //reflectWrite(v)(VertexSetData(v,data))
}
...
trait ScalaGenVertexOps extends BaseGenVertexOps with ScalaGenBase {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = {
    rhs match {
      ...
      case VertexSetData(v,d) => emitValDef(sym, quote(v) + "._data = " + quote(d))            
