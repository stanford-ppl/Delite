...
trait VectorOpsInlinedExp extends VectorOpsInlined {
  ...
  //def vector_new[A : Manifest](is_row: Exp[Boolean], len: Exp[Int]) : Exp[Vector[A]] = VectorNew(ETuple2(is_row, len)).asInstanceOf[Exp[Vector[A]]]
  //def vector_new[A : Manifest](is_row: Exp[Boolean], len: Exp[Int]) : Exp[Vector[A]] = new ApplyExtractor[(Boolean,Int),Vector[A]](vector_new_impl[A])(toAtom(ETuple2(is_row,len)))
