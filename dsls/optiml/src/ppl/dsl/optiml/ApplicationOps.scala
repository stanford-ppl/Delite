...
trait BinarizedGradientPyramidOpsExp extends BinarizedGradientPyramidOps with EffectExp {
  ...
  override def mirror[A:Manifest](e: Def[A], f: Transformer): Exp[A] = (e match {
    case Reflect(BinarizedGradientPyramidObjectNew(p,s,l,i), u, es) => reflectMirrored(Reflect(BinarizedGradientPyramidObjectNew(f(p),f(s),f(l),f(i)), mapOver(f,u), f(es)))
