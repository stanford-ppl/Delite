...
trait StreamOpsExp extends StreamOps with VariablesExp {
  ...
  */
  ...
  override def mirror[A:Manifest](e: Def[A], f: Transformer): Exp[A] = (e match {
    case e@StreamRawElem(x, idx) => reflectPure(StreamRawElem(f(x),f(idx)))
    case e@StreamChunkElem(x, idx, j) => reflectPure(new { override val original = Some(f,e) } with StreamChunkElem(f(x),f(idx),f(j)))
