...
trait OptiML2ScalaOpsPkgExp extends OptiML2ScalaOpsPkg with BaseFatExp with OrderingOpsExp with MiscOpsExp with IfThenElseExp with RangeOpsExp with PrimitiveOpsExp with ImplicitOpsExp with WhileExp
...
trait OptiML2Exp extends OptiML2ScalaOpsPkgExp with VectorOpsExp2 with MatrixOpsExp2
  ...
  def getCodeGenPkg(t: Target{val IR: OptiML2Exp.this.type}) : GenericFatCodegen{val IR: OptiML2Exp.this.type} = {
    t match {
...
trait OptiML2CodeGenBase extends GenericFatCodegen {
