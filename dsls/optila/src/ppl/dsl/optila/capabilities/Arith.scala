...
trait ArithOps extends Variables with OverloadHack {
  ...
  def infix_/[T,B](lhs: Rep[T], rhs: B)(implicit f: Fractional[T], c: B => Rep[T], mT: Manifest[T], ctx: SourceContext) = arith_fractional_divide(lhs,c(rhs))
...
//  def infix_exp[T](lhs: Rep[T])(implicit a: Arith[T], mT: Manifest[T]) = a.exp(lhs)
  ...
  implicit def denseVectorArith[T:Arith:Manifest]: Arith[DenseVector[T]] = new Arith[DenseVector[T]] {
    ...
    //                                               else a+b
    def +=(a: Rep[DenseVector[T]], b: Rep[DenseVector[T]])(implicit ctx: SourceContext) = a += b 
    def +(a: Rep[DenseVector[T]], b: Rep[DenseVector[T]])(implicit ctx: SourceContext) = a+b
    def -(a: Rep[DenseVector[T]], b: Rep[DenseVector[T]])(implicit ctx: SourceContext) = a-b
    def *(a: Rep[DenseVector[T]], b: Rep[DenseVector[T]])(implicit ctx: SourceContext) = a*b
    def /(a: Rep[DenseVector[T]], b: Rep[DenseVector[T]])(implicit ctx: SourceContext) = a/b

    def abs(a: Rep[DenseVector[T]])(implicit ctx: SourceContext) = a.abs
    def exp(a: Rep[DenseVector[T]])(implicit ctx: SourceContext) = a.exp
  ...
  implicit def matrixArith[T:Arith:Manifest] : Arith[Matrix[T]] = new Arith[Matrix[T]] {
    def +=(a: Rep[Matrix[T]], b: Rep[Matrix[T]])(implicit ctx: SourceContext) = a += b
    def +(a: Rep[Matrix[T]], b: Rep[Matrix[T]])(implicit ctx: SourceContext) = a+b
    def -(a: Rep[Matrix[T]], b: Rep[Matrix[T]])(implicit ctx: SourceContext) = a-b
    def *(a: Rep[Matrix[T]], b: Rep[Matrix[T]])(implicit ctx: SourceContext) = a*b
    def /(a: Rep[Matrix[T]], b: Rep[Matrix[T]])(implicit ctx: SourceContext) = a/b
    def abs(a: Rep[Matrix[T]])(implicit ctx: SourceContext) = a.abs
    def exp(a: Rep[Matrix[T]])(implicit ctx: SourceContext) = a.exp
    def empty(implicit ctx: SourceContext) = Matrix[T](0,0) // EmptyMatrix? 
