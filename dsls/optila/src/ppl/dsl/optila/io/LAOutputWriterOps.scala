...
trait LAOutputWriterOps extends Base {
  object LAOutputWriter {
    def write[A](m: Rep[Matrix[A]], filename: Rep[String])(implicit mA: Manifest[A], conv: Rep[A] => Rep[Double], ctx: SourceContext) = obj_laoutput_write(m,filename,conv)
    def writeVector[A](v: Interface[Vector[A]], filename: Rep[String])(implicit mA: Manifest[A], conv: Rep[A] => Rep[Double], ctx: SourceContext) = obj_laoutput_write_vector(v,filename,conv)
  }

  def obj_laoutput_write[A:Manifest](m: Rep[Matrix[A]], filename: Rep[String], conv: Rep[A] => Rep[Double])(implicit ctx: SourceContext): Rep[Unit]
...
trait LAOutputWriterOpsExp extends LAOutputWriterOps { this: LAOutputWriterImplOps with DeliteOpsExp =>
  ...
  def obj_laoutput_write[A:Manifest](m: Exp[Matrix[A]], filename: Exp[String], conv: Exp[A] => Exp[Double])(implicit ctx: SourceContext) = reflectEffect(LAOutputWrite(m,filename,conv))
