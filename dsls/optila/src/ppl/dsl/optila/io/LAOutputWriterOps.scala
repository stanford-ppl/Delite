...
trait LAOutputWriterOps extends DSLType with Base {
  object LAOutputWriter {
    ...
    def writeVector[A](v: Rep[Vector[A]], filename: Rep[String])(implicit mA: Manifest[A], conv: Rep[A] => Rep[Double]) = obj_laoutput_write_vector(v,filename,conv)
  }

  def obj_laoutput_write[A:Manifest](m: Rep[Matrix[A]], filename: Rep[String], conv: Rep[A] => Rep[Double]): Rep[Unit]
  def obj_laoutput_write_vector[A:Manifest](v: Rep[Vector[A]], filename: Rep[String], conv: Rep[A] => Rep[Double]): Rep[Unit]
}
...
trait LAOutputWriterOpsExp extends LAOutputWriterOps { this: LAOutputWriterImplOps with DeliteOpsExp =>
  ...
  case class LAOutputWriteVector[A:Manifest](v: Exp[Vector[A]], filename: Exp[String], conv: Exp[A] => Exp[Double]) extends DeliteOpSingleTask(reifyEffects(laoutput_write_vector_impl(v,filename,conv)))
  ...
  def obj_laoutput_write_vector[A:Manifest](v: Exp[Vector[A]], filename: Exp[String], conv: Exp[A] => Exp[Double]) = reflectEffect(LAOutputWriteVector(v,filename,conv))
}
