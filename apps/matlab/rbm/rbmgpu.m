function exectime = rbmgpu(datafile, numhid, numcases)
% Version 1.000 
%
...
vishid     = gpuArray(single(0.1)*randn(numdims, numhid, 'single'));
hidbiases  = zeros(1,numhid, 'single');
visbiases  = zeros(1,numdims, 'single');

poshidprobs = gpuArray(zeros(numcases,numhid, 'single'));
neghidprobs = gpuArray(zeros(numcases,numhid, 'single'));
posprods    = gpuArray(zeros(numdims,numhid, 'single'));
negprods    = gpuArray(zeros(numdims,numhid, 'single'));
vishidinc  = gpuArray(zeros(numdims,numhid, 'single'));
hidbiasinc = gpuArray(zeros(1,numhid, 'single'));
visbiasinc = gpuArray(zeros(1,numdims, 'single'));
...
%%%%%%%%% START NEGATIVE PHASE  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  neghidact = sum(gather(neghidprobs));
  negvisact = sum(gather(negdata)); 

%%%%%%%%% END OF NEGATIVE PHASE %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  err= sum(sum( gather((data-negdata).^2) ));
  errsum = err + errsum;
...
%%%%%%%%% UPDATE WEIGHTS AND BIASES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    ...
    visbiases = gather(visbiases + visbiasinc);
    hidbiases = gather(hidbiases + hidbiasinc);
